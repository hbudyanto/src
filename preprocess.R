...
## author : HAB, 31 July 2016

#########################################
## ========== Clean Up Transaction Data
#########################################
# Seth Working Path (should be properly assigned into the right order)
...
# no longer needed later with full data set
...
report1 = report1[which(report1$customer_id %in% key.cust), ]
...
load('report2.Rdata') 
report2 = report2[which(report2$customer_id %in% key.cust),]
...
report1$customer_id <- tmp[as.character(report1$customer_id), 2]

write.csv(report2, 'data/web2.csv', row.names = F, col.names = T, sep=',')
write.csv(report1, 'data/web1.csv', row.names = F, col.names = T, sep=',')
write.csv(cust, 'data/cust.csv', row.names = F, col.names = T, sep=',')
write.csv(dotmailer, 'data/dotmailer.csv', row.names = F, col.names = T, sep=',')
